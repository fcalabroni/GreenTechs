...
#%% BASELINE with sensitivity on Weibull distribution (RR = hist, price = Avg)
...
for scen in ['Baseline']:   
    for year in years:
        for s in sens:
            ...
            path_A2 = f"{pd.read_excel(paths, index_col=[0]).loc['A2',user]}\\Baseline\\A2_{s}_hist.xlsx"
            path_A2_act = f"{pd.read_excel(paths, index_col=[0]).loc['A2',user]}\\Act\\A2_act_{s}_hist.xlsx"
            ...
            if scen == 'Baseline':
                ...
                # extraction of data about minerals and conversion from M€ to tons of minerals
...
for s in sens:
    ...
    with pd.ExcelWriter(f"{pd.read_excel(paths, index_col=[0]).loc['Results',user]}\\Sensitivity\\Consumption\\Lifetime\\cons_gt_{s}.xlsx") as writer:
